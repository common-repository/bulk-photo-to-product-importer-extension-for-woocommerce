...
class PTPImporter_Product {
   ...
   public function create( $posted ) {
    ...
    public function create_variations( $term_id, $parent_id, $file_data ) {
        ...
        foreach ( $group->variations as $variation ) {
            ...
            //if ( $variation['name'] == 'Downloadable' || $variation['name'] == 'downloadable' ) {
                ...
                // Set as downloadable
                $metadata['_downloadable'] = $variation['_downloadable'];
				// Set as virtual'
                ...
                $transient_name = 'wc_product_children_ids_' . $parent_id . $version_value;
        ...
        for ( $i = 0; $i < count($parents); $i++ ) {
            foreach ( $variations as $variation ) {
                ...
                //if ( $variation['name'] == 'Downloadable' || $variation['name'] == 'downloadable' ) {
                    ...
                    // Set as downloadable
                    $metadata['_downloadable'] = isset($variation['downloadable']) ? 'yes' : 'no';
                    // Set download path
                    ...
                    // Add this to cache. This fixes the issue of this downloadable product not being included in the front-end
                    $transient_name = 'wc_product_children_ids_' . $parents[$i]->ID . $version_value;
                    $children = (array)get_transient( $transient_name );
                    $children[] = $post_id;
                    set_transient( $transient_name, $children );
                //}
    ...
    public function testImageWaterMark($attachment_id, $file_loc) {
        global $ptp_importer;
        update_post_meta($attachment_id, $ptp_importer->attachment_meta_key, 'yes');
        $attach_data = wp_generate_attachment_metadata($attachment_id, $file_loc);
        wp_update_attachment_metadata($attachment_id, $attach_data);
        return array('success'=>true, 'variations'=>$variations, 'file_id'=>$attachment_id);
    }
    /**
