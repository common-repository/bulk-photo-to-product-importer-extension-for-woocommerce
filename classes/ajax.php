...
class PTPImporter_Ajax
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_nopriv_ptp_remove_cart_item', array($this, 'remove_cart_item'));
        add_action('wp_ajax_ptp_remove_cart_item', array($this, 'remove_cart_item'));

        // Settings > Variations fix
        add_action('wp_ajax_nopriv_ptp_fix_variations', array($this, 'fix_variations'));
        add_action('wp_ajax_ptp_fix_variations', array($this, 'fix_variations'));
    ...
    public function product_select()
    {
        ...
        $photos_obj = PTPImporter_Product::getInstance();
        $response = $photos_obj->testImageWaterMark($_POST['id'], $file_loc);
    ...
    public function products_add_to_cart()
    {
        ...
        foreach ($posted['ptp_grouped_products'] as $grouped_product_id) {
            ...
            foreach ($variations as $variationval) {
                ...
                $variation_db_name = get_the_title($variationval);
                $arrv[] = $variation_db_name;
                if ($varvalue == 'yes' && $variation_name == $variation_db_name /*&& ($variation_name == 'Downloadable' OR $variation_name == 'downloadable')*/) {
                    $product_id[] = $variationval;

                }else if ($varvalue == 'no' && ($variation_name != 'Downloadable') && $variation_name == $variation_db_name) {
                    $product_id[] = $variationval;
                }
...
//print_r($data); die("Die Before Cart-2"); 
    ...
    // AJAX path to remove a whole item (counter turned to zero)
    public function remove_cart_item() {
        $cart = WC()->instance()->cart;

        $_id = $_POST['id'];

        $cart_id = $cart->generate_cart_id($_id); // $_id as Post id
    ...
    /**
     * Fix broken variations on the options table
     */
    public function fix_variations() {
        global $wpdb;

        $variation_query = "SELECT * FROM {$wpdb->prefix}options WHERE `option_name` LIKE '_transient_wc_product_children_ids_%'";
        $variation_groups = $wpdb->get_results($variation_query, ARRAY_A);

        echo '<pre>' . print_r($variation_groups) . '</pre>';
    }
