<?php
/*
Plugin Name: WPPC Bulk Photo to Product Importer Extension for WooCommerce (Free)
Plugin URI: https://wordpress.org/plugins/bulk-photo-to-product-importer-extension-for-woocommerce/
...
defined( 'TPC_BPTPI_ROOT'      ) or define( 'TPC_BPTPI_ROOT'     , dirname( __FILE__ ) );
defined( 'TPC_BPTPI_URL'       ) or define( 'TPC_BPTPI_URL'      , plugins_url( basename( TPC_BPTPI_ROOT ) ) );
defined( 'TPC_BPTPI_SLUG') or define( 'TPC_BPTPI_SLUG', 'tpc-bptpi');
...
class PTP_Importer {
     ...
     * @var string
     */
    public $variation_is_downloadable_meta_key = '_downloadable';
    ...
    private function _init( ) {
		...
		add_option($this->settings_meta_key."_imagesetting",0);
		...
		$uploadersys = get_option($this->settings_meta_key."_uploader");
	   	if($uploadersys === false) {
			...
			self::$uploader_sys = "basic";
		}else{
			self::$uploader_sys = $uploadersys;
		}
    ...
    public function admin_scripts() {
        ...
        wp_enqueue_script( 'ptp_uploader', plugins_url( 'assets/js/'.self::$uploader_sys.'.upload.js', __FILE__ ), array('jquery', 'plupload-handlers'), null );
