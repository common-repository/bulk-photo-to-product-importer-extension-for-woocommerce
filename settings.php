						...
						<tbody>
							<tr>
								...
								<td>
									...
									<p><a class="ptp-button-primary" href="https://www.wppluginco.com/product/premium-bulk-photo-to-product-importer-extension-for-woocommerce" title="<?php echo $ptp_importer->plugin_name; ?>" target="_blank">Upgrade to premium version &raquo;</a></p>
								</td>
							</tr>
						</tbody>
					</table>
...
<!--						<tr>-->
<!--							<td class="label">-->
<!--								<label>--><?php //_e( 'Broken Variations', 'ptp' ); ?><!--</label>-->
<!--							</td>-->
<!--							<td>-->
<!--								<a href="#" id="ptp-variation-fix">Click here to fix variations</a>-->
<!--							</td>-->
<!--						</tr>-->
        ...
        <div class="wp-box">
			<?php
				$imagesetting = get_option($this->settings_meta_key."_imagesetting");
				if(isset($_GET["_imagesetting"]) && $_GET["_imagesetting"] == true){
					if($imagesetting == 1)
						update_option($this->settings_meta_key."_imagesetting",0);
					else
						update_option($this->settings_meta_key."_imagesetting",1);
				}
				$imagesetting = get_option($this->settings_meta_key."_imagesetting");
			?>
            <div class="title"><h3><?php _e( 'Images Settings', 'ptp' ); ?></h3></div>

                <table class="ptp-input widefat">
                    <tbody>
                        <!--<tr>
							<td class="label">
								<label>Images Setup</label>
                            </td>
                            <td>
								You are using <b><?php// if($imagesetting	==	1)	echo 'Advanced'; else echo 'Default'; ?></b>  WP Settings now. To remove creation of unnecessary image please click <a href="admin.php?page=ptp_settings&_imagesetting=true">here</a>
                            </td>
                        </tr>-->
                        <tr>
							<td class="label">
								<label><?php _e( 'Images Settings', 'ptp' ); ?></label>
							</td>
							<td>
								<input type="checkbox" id="" class="general-settings-input" name="imagesetting" value="<?php if($imagesetting	==	1)	echo 1; else echo 0; ?>" <?php checked( $imagesetting, 1 ); ?> />
								<label class="ptp-subtext" for=""><?php _e( 'Prevent WordPress from generating thumbnails. Note: When uploading an image through WordPress native Media Uploader multiple resolutions (thumbnails) are generated by WordPress based on the values in Settings > Media. The generation of these thumbnails is not necessary in the case of BPTPI, so we have disabled it by default. But in the case you need them generated you may de-select this checkbox.' ); ?></label>
							</td>
						</tr>
                        <tr>
							<td class="label">
							...
							<td>
								<a href="admin.php?page=ptp_settings&_imagesetting=true"><input type="submit" class="ptp-button-primary" value="<?php _e( $submit_button_label, 'ptp'); ?>" /></a>
							</td>
						</tr>
                    </tbody>
                </table>
	...
	$im_plugins_list = array(
		...
		'ACF Front End Form Plugin' => 'https://www.iteratemarketing.com/product/acf-front-end-form-plugin',
		'CRM Plugin For Wordpress' => 'https://www.iteratemarketing.com/product/crm-plugin-for-wordpress',
		'Custom Pointers Plugin For Wordpress' => 'https://www.iteratemarketing.com/product/custom-pointers-plugin-for-wordpress'
	);
	?>
	<div id="ptp-col-right">
		<div class="wp-box">
			<div class="title"><h3><?php _e( 'Info', 'ptp' ); ?></h3></div>
			<table class="ptp-input widefat">
				<tbody>
					<tr>
						<td>
							This Plugin was created by <a href="https://www.wppluginco.com/">WP Plugin Company</a> and <a href="https://www.wppluginco.com/services/wordpress-plugins">you can find more of our Plugins, or request a custom one, here </a>.
							<?php
