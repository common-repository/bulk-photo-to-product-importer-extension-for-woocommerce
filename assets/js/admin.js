;(function (w, $) {
    var PTPImporter = {
        ...
        init: function () {
            ...
            $( document ).on('click', '#ptp-variation-fix', this.GeneralSettings.fixVariations);
        ...
        Variations: {
            ...
            addRow: function (e) {
                var variationCount = $( '.variations-form .variation-count' ).val() ? parseInt( $( '.variations-form .variation-count' ).val() ) : 0,
                    ...
                    template = $('<table class="row">' +
                                 ...
                                 '</td>' +
                                 '<td>' +
                                 '<input type="checkbox" name="variations['+ variationCount +'][downloadable]" checked="checked" style="height: 16px; width: 16px;"/>' +
                                 '</td>' +
                                 '<td>' +
            ...
            update: function (e) {
                ...
                $.post(PTPImporter_Vars.ajaxurl, data, function(res) {
                    ...
                    // Request separator for escaping server corruptions
                    var responseBracketStart = '_JSON_START_';
                    var responseBracketEnd = '_JSON_END_';

                    if (res.indexOf('_JSON_START_') == -1) {
                        return;
                    }

                    res = res.substring(  res.indexOf( responseBracketStart ) + responseBracketStart.length, res.indexOf( responseBracketEnd ) );
        ...
        GeneralSettings: {
            ...
            fixVariations: function(e) {
                e.preventDefault();

                var that = $(this);
                var data = {
                    action: 'ptp_fix_variations'
                }
                ...
                console.log(PTPImporter_Vars.ajaxurl);
            }
        }
    };
