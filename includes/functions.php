...
function ptp_generate_watermaked_images( $metadata = array(), $attachment_id ) {
    ...
    $metadata['file'] = trim($uploads_dir['subdir'], "/") . '/' . basename( $outpout_file_path );
...
add_filter( 'wp_calculate_image_srcset', 'customizedSrcSet', 10, 5 );
function customizedSrcSet($sources, $size_array, $image_src, $image_meta, $attachment_id) {
    ...
    $relative_path = substr( $image_src, strpos($image_src, 'uploads') + 7 );
    if(is_multisite() && get_current_blog_id() > 1) {

        $searchStr = 'uploads/sites/' . get_current_blog_id();

        $siteLength = strlen((string)get_current_blog_id());

        $relative_path = substr($image_src, strpos($image_src, $searchStr) + (14+$siteLength));

    }

    $subDirectoryTemp = explode("/", $relative_path);

    $subDirectory = '';

    foreach($subDirectoryTemp as $subKey => $sub) {

        if($subKey < (count($subDirectoryTemp) -2))
            $subDirectory .= $sub . '/';
        else if($subKey < count($subDirectoryTemp) -1)
            $subDirectory .= $sub;
    }
    ...
    foreach($sources as $sourceKey => $source) {
        ...
        $uploadSubPath = $uploadDir['basedir'];
        $tempUpload = substr($uploadDir['baseurl'], strpos($uploadDir['baseurl'], 'wp-content') + 11);
        if(!file_exists( $uploadSubPath . substr( $source['url'], strpos($source['url'], $tempUpload) + strlen($tempUpload) ))) {

            $tempUrl = explode("/", $source['url']);

            $tempFile = end($tempUrl);

            $tempUrl[count($tempUrl) -1] = ltrim($subDirectory, '/') . '/' . $tempFile;

            $sources[$sourceKey]['url'] = implode("/", $tempUrl);
        }
    }
